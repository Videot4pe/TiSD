...
{
    if (tree->left)
        print_b_tree(tree->left, n + 5);
    for (int i = 0; i < n; i++)
        printf("  ");
    printf("%s\n\n", tree->data);
    if (tree->right)
        print_b_tree(tree->right, n + 5);
}
...
int main()
{
    ...
    FILE *f1 = fopen("/Users/garanya/stuff/Программирование/xcode/lab007/lab007/tree.txt", "r");
    char * buf = malloc(sizeof(char)*LEN);
    //char * global = malloc(sizeof(char)*(LEN+128));
    //global = " ";
