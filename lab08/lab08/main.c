...
void floydWarshall(int **matrix, int numberOfVert)
{
    for(int k = 0; k < numberOfVert; k++)
        for(int i = 0; i < numberOfVert; i++)
            for(int j = 0; j < numberOfVert; j++)
                matrix[i][j] = minBetween(matrix[i][j], matrix[i][k] + matrix[k][j]);
    return;
}
...
void dfs(int **matrix, int numberOfVert, int start, int *visited)
{
    ...
    for (int j = start; j < numberOfVert; j++)
        if (matrix[start][j] != INF && start != j)
            dfs(matrix, numberOfVert, j, visited);
}

int main(int argc, char** argv) {
    FILE *f;
    f = fopen("/Users/garanya/Dropbox/Учеба/ТИСД/lab08/lab08/matrix.txt", "r");
    int numberOfVert;
...
//    int start = 0;
//    printf("Введите начальную вершину остовного дерева минимальной стоимости: ");
//    scanf("%d", &start);
...
//    printf("-------------------------------------------\n");
    ...
    printf("Граф кратчайших путей между всеми пунктами: \n");
    printMatrix(matrix, numberOfVert);
